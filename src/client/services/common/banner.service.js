    ...
    if (type === Tw.REDIS_BANNER_TYPE.TOS) {
      ...
      return _.chain(banners)
        .filter(function(banner) { 
          return (
            ...
            (!banner.expsStaDtm || Tw.DateHelper.getDiffByUnit(Tw.DateHelper.convDateFormat(banner.expsStaDtm || '00'), today, 'minute') <= 0) && // not yet exposure dat e
            (!banner.expsEndDtm || Tw.DateHelper.getDiffByUnit(Tw.DateHelper.convDateFormat(banner.expsEndDtm || '00'), today, 'minute') >= 0)  // end of exposure date
          );
        })
        .sort(function(a, b) {
          return Number(a.bnnrExpsSeq) - Number(b.bnnrExpsSeq);
      ...
      return _.chain(banners)
        .filter(function(banner) { 
          ...
          return (
            (banner.chnlClCd.indexOf(Tw.REDIS_DEVICE_CODE.MOBILE) >= 0 || banner.chnlClCd.indexOf(browserCode) >= 0) && // only mobile
            (!banner.expsStaDtm || Tw.DateHelper.getDiffByUnit(Tw.DateHelper.convDateFormat(banner.expsStaDtm || '00'), today, 'minute') <= 0) && // not yet exposure date
            (!banner.expsEndDtm || Tw.DateHelper.getDiffByUnit(Tw.DateHelper.convDateFormat(banner.expsEndDtm || '00'), today, 'minute') >= 0)  // end of exposure date
          );
        })
        .sort(function(a, b) {
