...
Tw.NativeService.prototype = {
  ...
  _onBack: function () {
    ...
    if ( !Tw.FormatHelper.isEmpty($popupContainer) ) {
      ...
      if ( /\/en\/main\/home/.test(location.href) || /\/en\/main\/store/.test(location.href) ) {
  ...
  _onEasyLogin: function (resp) {
    if ( resp.resultCode === Tw.NTV_CODE.CODE_00 ) {
      if ( Tw.BrowserHelper.isAndroid() ) {
        window.location.href = '/en/common/member/slogin/aos?mdn=' + resp.params.mdn;
      } else {
        window.location.href = '/en/common/member/slogin/ios';
      }
    } else {
      // error
    }
