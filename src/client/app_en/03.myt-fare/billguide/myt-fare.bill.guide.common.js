...
Tw.MyTFareBillGuideCommon.prototype = {
  _init: function () {
   ...
   this._renderBillGroup(this.resData.lineDtlLst, false, this.$container);
  ...
  _bindEvent: function () {
    ...
    // this.$container.on('click', '[data-target="callGiftBtn"]', $.proxy(this._callGiftBtnEvt, this)); // 콜기프트 사용요금        --영문화
    // this.$container.on('click', '[data-target="roamingBtn"]', $.proxy(this._roamingBtnEvt, this)); // 로밍 사용요금              --영문화
    // this.$container.on('click', '[data-target="donationBtn"]', $.proxy(this._donationBtnEvt, this)); // 기부금/후원금 사용요금    --영문화

    // this.$container.on('click', '[data-target="feePayBtn"]', $.proxy(this._feePayBtnEvt, this)); // 요금납부                      --영문화
    // this.$container.on('click', '[data-target="payListBtn"]', $.proxy(this._payListBtnEvt, this)); // 납부내역조회                 --영문화

    // this.$container.on('click', '[data-target="detailContentsBtn"]', $.proxy(function() { // 콘텐츠 이용료 최초화면 바로가기         --영문화
    //   this._history.goLoad('/myt-fare/bill/contents/history');
    // }, this));

    // this.$container.on('click', '[data-target="detailMicroBtn"]', $.proxy(function() { // 소액결재 최초화면 바로가기                   --영문화
    //   this._history.goLoad('/myt-fare/bill/small/history');
    // }, this));

    // this.$container.on('click', '[data-target="childBillInfo"]', $.proxy(this._goChildBillInfo, this)); // 자녀사용량 조회화면으로 이동  --영문화
    // this.$container.on('click', '[data-target="goProdPageBtn"]', $.proxy(this._goProdPage, this)); // 상품원장페이지로 이동               --영문화
  },
  //--------------------------------------------------------------------------[EVENT]
  /**
   * 상품원장 페이지로 이동
