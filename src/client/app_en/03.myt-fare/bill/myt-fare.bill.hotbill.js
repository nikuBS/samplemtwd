...
Tw.MyTFareHotBill.prototype = {
  ...
  _init: function () {
    ...
    this._bindEvent();
    this._getSvcInfo();
    // if ( this._lines.length > 0 ) {                                                                  //영문화 다른회선조회 제거.
    //   this._renderLines();
    ...
    /* this._lineComponent = */ new Tw.LineComponent(this.$container, '.fe-bt-line', true, null);   //영문화
  },
  /**
   * @function
  ...
  //  this.$preBill = this.$container.find('#fe-bt-prev');    --영문화 사용안함
  //  this.$btMore = this.$container.find('#fe-bt-more');     --영문화 사용안함
  //  this.$lineList = this.$container.find('#fe-line-info'); --영문화 사용안함
  },
  /**
   * @function
   ...
   */
  _bindEvent: function () {
  //  this.$preBill.on('click', $.proxy(this._onClickPreBill, this));                 --영문화
  //  this.$container.on('click', '#fe-bt-more', $.proxy(this._renderLines, this));   --영문화
  },
  /**
...
//          .request(Tw.API_CMD.BFF_05_0022, params)
          .request(Tw.API_CMD.BFF_05_0223, params)
...
//       this.$amount.text(total + Tw.CURRENCY_UNIT.WON);
      ...
      //this.$period.text(this.$period.text() + fromDt + ' ~ ' + toDt); --영문화 포멧변경
        this.$period.text( fromDt + ' ~ ' + toDt);
        var fieldInfo = {
          lcl: 'billItmLclNm',
          scl: 'billItmSclNm',
          name: 'billItmNm',
...
Tw.MyTFareHotBill.arrayToGroup = function (data, fieldInfo) {
  ...
  data.forEach(function (item) {
    ...
    var bill_item = {
      ...
      amount: (amount < 0 ?'-₩':'₩')+Tw.StringHelper.commaSeparatedString( Math.abs(amount) ),
  ...
  $.each(group, function (key1, itemL) {
    $.each(itemL, function (key2, itemS) {
      if ( groupInfoFields.indexOf(key2) < 0 ) {
        ...
        itemS.total = (itemS.total < 0 ?'-₩':'₩')+Tw.StringHelper.commaSeparatedString( Math.abs(itemS.total) );
      }
    });
    
    itemL.discount = itemL.total < 0;
    itemL.total = (itemL.total < 0 ?'-₩':'₩')+Tw.StringHelper.commaSeparatedString( Math.abs(itemL.total) );
  });
  return group;
};
