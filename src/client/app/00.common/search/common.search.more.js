...
$.extend(Tw.CommonSearchMore.prototype,
{
   ...
   */
  _nextInit : function () {
    Tw.Logger.info('[common.search.more] [_nextInit]', 'this._paramObj : ' + this._paramObj);   // {keyword: "요금제", step: "3", category: "rate", sort: "A"}
    ...
    Tw.Logger.info('[common.search.more] [_nextInit]', 'this._searchInfo.search[0][this._category] : ' + this._searchInfo.search[0][this._category]);
  ...
  _onOpenGradeActionSheet: function () {
    ...
    $container.find('li input').change($.proxy(function (event) {
      ...
      Tw.Logger.info('[common.search.more] [_onOpenGradeActionSheet]', 'substring start index : ' + this._reqOptions.sortCd.indexOf(selectedCollection + '-'));

      var startIdx = this._reqOptions.sortCd.indexOf(selectedCollection + '-') + selectedCollection.length + 1;      
      Tw.Logger.info('[common.search.more] [_onOpenGradeActionSheet]', selectedCollection + ' 컬렉션의 변경된 정렬기준 : ' + this._reqOptions.sortCd.substring(startIdx, startIdx+1));
  ...
  _sortRate : function (options) {
    ...
    .done($.proxy(function (res) {
      if (res.code === 0) {
        ...
        Tw.Logger.info('[common.search.more] [_sortRate] res.result.search[0]', res.result.search[0]);
    ...
    if (query !== researchQuery) {
      ...
      if(Tw.BrowserHelper.isApp()) {
        if(Tw.BrowserHelper.isAndroid()) {
          reqOptions = {query: encodeURIComponent(query), collection: 'all', researchCd: 1, sort: 'shortcut-A.rate-A.service-A.tv_internet-A.troaming-A.tapp-D.direct-D.tmembership-R.event-D.sale-C.as_outlet-R.question-D.notice-D.prevent-D.manner-D.serviceInfo-D.siteInfo-D.bundle-A', researchQuery: encodeURIComponent(researchQuery), device: 'A'};
        } else {
          reqOptions = {query: encodeURIComponent(query), collection: 'all', researchCd: 1, sort: 'shortcut-A.rate-A.service-A.tv_internet-A.troaming-A.tapp-D.direct-D.tmembership-R.event-D.sale-C.as_outlet-R.question-D.notice-D.prevent-D.manner-D.serviceInfo-D.siteInfo-D.bundle-A', researchQuery: encodeURIComponent(researchQuery), device: 'I'};
        }
      } else {
        reqOptions = {query: encodeURIComponent(query), collection: 'all', researchCd: 1, sort: 'shortcut-A.rate-A.service-A.tv_internet-A.troaming-A.tapp-D.direct-D.tmembership-R.event-D.sale-C.as_outlet-R.question-D.notice-D.prevent-D.manner-D.serviceInfo-D.siteInfo-D.bundle-A', researchQuery: encodeURIComponent(researchQuery)};
      }
    } else {
      if(Tw.BrowserHelper.isApp()) {
        if(Tw.BrowserHelper.isAndroid()) {
          reqOptions = {query: encodeURIComponent(query), collection: 'all', sort: 'shortcut-A.rate-A.service-A.tv_internet-A.troaming-A.tapp-D.direct-D.tmembership-R.event-D.sale-C.as_outlet-R.question-D.notice-D.prevent-D.manner-D.serviceInfo-D.siteInfo-D.bundle-A', device: 'A'};
        } else {
          reqOptions = {query: encodeURIComponent(query), collection: 'all', sort: 'shortcut-A.rate-A.service-A.tv_internet-A.troaming-A.tapp-D.direct-D.tmembership-R.event-D.sale-C.as_outlet-R.question-D.notice-D.prevent-D.manner-D.serviceInfo-D.siteInfo-D.bundle-A', device: 'I'};
        }
      } else {
        reqOptions = {query: encodeURIComponent(query), collection: 'all', sort: 'shortcut-A.rate-A.service-A.tv_internet-A.troaming-A.tapp-D.direct-D.tmembership-R.event-D.sale-C.as_outlet-R.question-D.notice-D.prevent-D.manner-D.serviceInfo-D.siteInfo-D.bundle-A'};
      }
    }
        ...
        for(var i=0;i<res.result.search.length;i++){
          ...
          if(keyName==='smart'||keyName==='immediate'||keyName==='banner'||keyName==='lastevent'){
            ...
            if ( contentsCnt < 1 ) {
              ...
              var sort = this._reqOptions.sortCd.substring(this._reqOptions.sortCd.indexOf(keyName + '-') + keyName.length + 1, this._reqOptions.sortCd.indexOf(keyName + '-') + keyName.length + 2);
              ...
              if (query !== researchQuery) {
                ...
                this.$container.find('.fe-category.all').attr('href', '/common/search?category=all&keyword=' + query + '&step=' + (this._step + 1)  + '&in_keyword=' + researchQuery  + '&sort=' + sort + '&redirect=N');
                this.$container.find(categoryStr).attr('href', '/common/search/more?category=' + keyName + '&keyword=' + query + '&step=' + (this._step + 1)  + '&in_keyword=' + researchQuery  + '&sort=' + sort);
              } else {
                ...
                this.$container.find('.fe-category.all').attr('href', '/common/search?category=all&keyword=' + query + '&step=' + (this._step + 1) + '&sort=' + sort + '&redirect=N');
                this.$container.find(categoryStr).attr('href', '/common/search/more?category=' + keyName + '&keyword=' + query + '&step=' + (this._step + 1) + '&sort=' + sort);
              }
            }
