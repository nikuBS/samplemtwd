...
Tw.CommonSearch.prototype = {
  ...
  _goLink : function (linkEvt) {
    ...
    this._apiService.request(Tw.API_CMD.STACK_SEARCH_USER_CLICK,
      ...
      //'docId='+$linkData.data('id')+'&section='+$linkData.data('category')+'&title='+$linkData.data('tit')+'&keyword='+this._searchInfo.researchQuery
      {
        'docId' : $linkData.data('id'),
        'section' : $linkData.data('category'),
        'title' : $linkData.data('tit'),
        'keyword' : this._searchInfo.researchQuery
      }
    );
