...
Tw.MyTJoinPhoneNumWireChange = function ($rootEl, data) {
  ...
  this._onProcess = this._data['wireSvcOperStCd'] && this._data['wireSvcOperStCd'] === '3';
  this._bindEvent();
};
...
Tw.MyTJoinPhoneNumWireChange.prototype = {
  ...
  _onclickSchNums: function (e) {
    ...
    if ( !param.staLineNum || param.staLineNum.length !== 4 || !param.endLineNum || param.endLineNum.length !== 4 ) {
  ...
  _requestChangeNumber: function () {
    this._apiService.request(Tw.API_CMD.BFF_05_0211,
      { afterSvcNum: this._options.to.replace(/-/gi, '') }, {}, [this._data.svcMgmtNum])
