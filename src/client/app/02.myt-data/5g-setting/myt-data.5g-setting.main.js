...
Tw.MyTData5gSettingMain.prototype = {
  ...
  _cachedElement: function () {
    this.$content = this.$container.find('.5gx-contents');
  ...
  _bindEvent: function () {
    ...
    // 시간설정다이얼 동적 이동 시 이벤트 처리
    this.$datePicker.circle_datepicker().on('timer', $.proxy(this._onTimer, this));
    // 조회하기 버튼 선택
   ...
   */
  _clearPage: function () {
    var template = Handlebars.compile(Tw.MYT_TPL.DATA_SUBMAIN['5GX_CLEAR_TEMP']);
