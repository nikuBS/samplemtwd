...
Tw.MyTDataPrepaySKPaySdk.prototype = {
  ...
  _onSuccessSkpayAuth: function (resp) {
    ...
    if ( resp.code === Tw.API_CODE.CODE_00 ) {
      var _redirectUri = this._data.skpayInfo.redirectUri + '?dataKey=' + resp.result.orderNumber + '&source=' +
        this._data.title + '&mb=' + this._data.skpayInfo.mb + '&amtCd=' + this._data.skpayInfo.amtCd + '&sms=' +
        this._data.skpayInfo.sms + '&email=' + this._data.skpayInfo.email;
      if ( this._data.title === 'voice' ) {
        _redirectUri = this._data.skpayInfo.redirectUri + '?dataKey=' + resp.result.orderNumber + '&source=' +
          this._data.title + '&previousAmount=' + this._data.skpayInfo.previousAmount + '&afterAmount=' +
          this._data.skpayInfo.afterAmount + '&rechargeAmount=' + this._data.skpayInfo.rechargeAmount + '&sms=' +
          this._data.skpayInfo.sms + '&email=' + this._data.skpayInfo.email;
      }
      /* jshint ignore:start */
