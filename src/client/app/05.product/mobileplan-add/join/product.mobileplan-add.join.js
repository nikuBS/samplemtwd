...
Tw.ProductMobileplanAddJoin.prototype = {
  ...
  _procJoinRes: function(resp) {
    ...
    if ( this._prodId == 'NA00006811') {
      this._apiService.request(Tw.API_CMD.BFF_10_0181, {
        smsPhrsGrpId: 'SMART'
      }, {}, ['NA00006810']); // SMS 발송 API 파라미터는 본상품 코드로 호출 필요, Swing 상품 측 요건으로 OP002-435 댓글 참조
    }

    // Swing 문자 발송 API 호출 (분실안심990++_예약 가입 시)
    // TODO: 특정 상품코드에 대한 하드코딩 개선 필요
    if ( this._prodId == 'NA00006978') {
      this._apiService.request(Tw.API_CMD.BFF_10_0181, {
        smsPhrsGrpId: 'SMART'
      }, {}, ['NA00006977']); // SMS 발송 API 파라미터는 본상품 코드로 호출 필요, Swing 상품 측 요건으로 OP002-7337 댓글 참조
    }
    ...
    if ( this._prodId == 'NA00006395') {
      this._apiService.request(Tw.API_CMD.BFF_10_0181, {
        smsPhrsGrpId: 'SMART'
      }, {}, ['NA00006397']); // SMS 발송 API 파라미터는 본상품 코드로 호출 필요, Swing 상품 측 요건으로 OP002-7693 댓글 참조
    }
