...
Tw.TidLandingComponent.prototype = {
  ...
  _bindEvent: function (redirectTarget) {
    ...
    this.$container.on('click', '.fe-bt-signup', $.proxy(this._onClickSignup, this, redirectTarget));
  },
  ...
  _goLoad: function (nativeCommand, url, callback) {
    if ( Tw.BrowserHelper.isApp() ) {
      if(Tw.NTV_CMD.CHANGE_PW === nativeCommand) //비밀번호 변경 국문페이지로 처리
        this._nativeService.send(nativeCommand, {}, callback);
      else
        this._nativeService.send(nativeCommand, {isForeigner:'Y'}, callback);
    } else {
  ...
  _onClickReplaceLogin: function (target) {
    ...
    if ( Tw.BrowserHelper.isApp() && Tw.FormatHelper.isEmpty(Tw.CommonHelper.getCookie('TWM')) ) {
      ...
      this._goLoad(Tw.NTV_CMD.LOGIN, '/en/common/tid/login?target=' + encodeURIComponent(target) + '&type=reload',
        $.proxy(this._onNativeLogin, this, target));
    }
  },
   ...
   * @private
   */
  _onClickSignup: function (target) {
    this.goSignup(target);
  },
  ...
  goLogin: function (target) {
    ...
    if ( Tw.BrowserHelper.isApp() && Tw.FormatHelper.isEmpty(Tw.CommonHelper.getCookie('TWM')) ) {
      ...
      this._goLoad(Tw.NTV_CMD.LOGIN, '/en/common/tid/login?target=' + encodeURIComponent(target), $.proxy(this._onNativeLogin, this, target));
    }
  },
  ...
  goSLogin: function (target) {
    ...
    if ( Tw.BrowserHelper.isApp() ) {
      if ( Tw.BrowserHelper.isAndroid() ) {
        this._historyService.goLoad('/en/common/member/slogin/aos?target=' + encodeURIComponent(target));
      } else {
        this._historyService.goLoad('/en/common/member/slogin/ios?target=' + encodeURIComponent(target));
      }
    }
  },
   ...
   * @desc 회원가입 요청
   */
  goSignup: function (target) {
    //var url = "/en/common/member/signup-guide"; // 돌려야 할 주소 
    target = target || '/en/main/home';
    this._goLoad(Tw.NTV_CMD.SIGN_UP, '/en/common/tid/signup-foreigner?target=' + encodeURIComponent(target), $.proxy(this._onNativeSignup, this));
  },
  ...
  goLogout: function () {
    if ( Tw.BrowserHelper.isApp() ) {
      this._nativeService.send(Tw.NTV_CMD.LOGOUT, {isForeigner:'Y'}, $.proxy(this._onNativeLogout, this));
    } else {
      this._historyService.goLoad('/en/common/tid/logout');
    }
  },
  ...
  _onClickBtAccount: function () {
    this._goLoad(Tw.NTV_CMD.ACCOUNT, '/en/common/tid/account', $.proxy(this._onNativeAccount, this));
  },
  ...
  _onClickBtFindId: function () {
    this._goLoad(Tw.NTV_CMD.FIND_ID, '/en/common/tid/find-id', $.proxy(this._onNativeFindId, this));
  },
  ...
  _onClickBtFindPw: function () {
    this._goLoad(Tw.NTV_CMD.FIND_PW, '/en/common/tid/find-pw', $.proxy(this._onNativeFindPw, this));
  },
  ...
  _onClickBtChangePw: function (target) {
    this._goLoad(Tw.NTV_CMD.CHANGE_PW, '/en/common/tid/change-pw?target=' + encodeURIComponent(target), $.proxy(this._onNativeChangePw, this));
  },
  ...
  _onNativeLogin: function (target, resp) {
    ...
    if (Tw.FormatHelper.isEmpty(resp) ) {
      this._historyService.replaceURL('/en/common/member/login/fail?errorCode=' + Tw.API_LOGIN_ERROR.FE0001);
    } else {
      if ( resp.resultCode === Tw.NTV_CODE.CODE_00 ) {
        ...
        if ( type === Tw.NTV_LOGINTYPE.ACTION_SHEET ) {
          this._historyService.replaceURL('/en/main/home');
        }
      } else {
        this._historyService.replaceURL('/en/common/member/login/fail?errorCode=' + resp.resultCode);
      }
    }
  },
  ...
  _successLogin: function (target, resp) {
    ...
    if ( resp.code === Tw.API_CODE.CODE_00 ) {
      ...
      // 휴면계정
      this._historyService.goLoad('/en/common/member/login/reactive?target=' + encodeURIComponent(target));
    } else if ( resp.code === Tw.API_LOGIN_ERROR.ATH1003 ) {
      this._historyService.replaceURL('/en/common/member/login/exceed-fail');
    } else if ( resp.code === Tw.API_LOGIN_ERROR.ATH3236 ) {
      this._historyService.goLoad('/en/common/member/login/lost?target=' + encodeURIComponent(target));
    } else {
      this._historyService.replaceURL('/en/common/member/login/fail?errorCode=' + resp.code);
    }
  },
  ...
  _successLogout: function (resp) {
    ...
    this._historyService.goLoad('/en/common/member/logout/complete');
