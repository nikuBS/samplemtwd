...
Tw.LineComponent.prototype = {
   ...
   * @param $target
   */
  onClickLineView: function (selectedMgmt, $target) {
    this._init(selectedMgmt);
    this._openLineList = true;
    this._getLineViewList($target);
  },
   ...
   * @private
   */
  _getLineViewList: function ($target) {
    this._apiService.requestArray([
      { command: Tw.NODE_CMD.GET_ALL_SVC, params: {}},
      // OP002-5303 : [개선][FE](W-1910-078-01) 회선선택 영역 확대
      { command: Tw.NODE_CMD.GET_CHILD_INFO, params: {}},
 ...
 /**
   ...
   * @private
   */
  _successGetLineViewList: function ($target, allSvcResp, childSvcResp, exposableResp) {
  ...
  _onClickLineButton: function () {
    this._historyService.replaceURL('/en/common/member/line');
  },
  ...
  _successChangeLine: function (resp) {
    ...
    if ( resp.code === Tw.API_CODE.CODE_00 ) {
      ...
      // 고객보호 비밀번호 설정페이지
      if ( this._openLineList ) {
        this._historyService.replaceURL('/en/myt-join/custpassword');
      } else {
        this._historyService.goLoad('/en/myt-join/custpassword');
      }
    } else {
      Tw.Error(resp.code, resp.msg).pop();
    }
