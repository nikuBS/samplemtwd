...
import { VOICE_UNIT } from '../types_en/bff.type';
...
class FormatHelper {
  ...
  static convVoiceFormat(data: any): any {
    ...
    const sec = FormatHelper.appendZeroTimeText(numHours);
  ...
  static appendZeroTimeText(time: number) {
    return (time < 9 ? '0' + time.toString() : time);
  }
}
