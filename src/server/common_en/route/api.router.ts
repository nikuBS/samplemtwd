...
import { API_CMD, API_CODE, API_METHOD } from '../../types_en/api-command.type';
...
import VERSION from '../../config_en/version.config';
import * as fs from 'fs';
import dateHelper from '../../utils_en/date.helper';
import DateHelper from '../../utils_en/date.helper';
import environment from '../../config_en/environment.config';
import BrowserHelper from '../../utils_en/browser.helper';
import { NODE_API_ERROR, TIME_UNIT, UNLIMIT_NAME } from '../../types_en/string.type';
import { TPLAN_SHARE_LIST, UNIT, UNIT_E, WIDGET_ERROR } from '../../types_en/bff.type';
import { COOKIE_KEY } from '../../types_en/common.type';
import { CHANNEL_CODE, MENU_CODE, REDIS_KEY, REDIS_TOS_KEY } from '../../types_en/redis.type';
...
import { SSO_SERVICE_LIST } from '../../types_en/config.type';
...
class ApiRouter {
  ...
  private getMenu(req: Request, res: Response, next: NextFunction) {
    ...
    this.logger.info(this, '[en>get menu]');
  ...
  private getAllSvcInfo(req: Request, res: Response, next: NextFunction) {
   // const loginService = new LoginService();
    ...
    res.json({
      code: API_CODE.CODE_00,
      result: apiService.getAllSvcInfo(req)
    });
  }
  ...
  private getChildInfo(req: Request, res: Response, next: NextFunction) {
    ...
    res.json({
      code: API_CODE.CODE_00,
      result: apiService.getChildInfo(req)
    });
  }
  ...
  private loginSvcPassword(req: Request, res: Response, next: NextFunction) {
    const apiService = new ApiService();
    const params = req.body;
    this.fillRoamingProperties(req, params);
    apiService.setCurrentReq(req, res);
  ...
  private loginTid(req: Request, res: Response, next: NextFunction) {
    ...
    apiService.setCurrentReq(req, res);
    apiService.requestLoginTid(params.tokenId, params.state, roamMcc).subscribe((resp) => {
