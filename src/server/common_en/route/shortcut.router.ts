...
import { SHORTCUT_LOGIN_TYPE } from '../../types_en/bff.type';
...
class ShortcutRouter {
  ...
  constructor() {
    ...
    this.router.get('*', (req, res, next) => {
      ...
      if ( dns === this.shortcutUrl ) {
        ...
        res.redirect('/en/common/auto-sms/cert?p=' + encParam);
      } else {
  ...
  private redirectTarget(req, res, next, target) {
    ...
    res.redirect('/en/common/share/bridge?' + 'target=' + encodeURIComponent(menuUrl) + '&loginType=' + loginType + '&referer=' + referer);
  }
