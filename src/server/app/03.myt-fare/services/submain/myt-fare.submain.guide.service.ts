...
/**
 * MenuName: 나의 요금 > 요금 안내서 서비브
 * @file myt-fare.bill.guide.service.ts
 * @author 양정규
 * @since 2020.11.05
 */
...
export class MytFareSubmainGuideService extends MytFareSubmainCommonService {
   ...
   * @desc 초기 설정
   */
  public init(req: Request, svcInfo: any, allSvc: any, childInfo: any, pageInfo: any): any {
   ...
   * PPS 선불폰
   */
  public getPrepaidCircuit(): Observable<any> {
    ...
    return this.apiService.request(API_CMD.BFF_05_0013, {invDt: reqQuery.date})
      .map((resp) => {
        ...
        const commDataInfo: CommDataInfo = Object.assign(this.defaultCommDataInfo(), {
          ...
          ppsCurDate : DateHelper.getShortDate(new Date()),
          ppsCurTime : DateHelper.getCurrentDateTime('HH:mm'),
   ...
   * @public
   */
  public getBillCharge(svcInfo: any, res: Response): Observable<any> {
   ...
   * @param pageInfo
   */
  public fail(res: Response, data: any): any {
    const {svcInfo, pageInfo} = this.info;
