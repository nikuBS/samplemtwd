...
import {DATA_UNIT, MYT_FEEPLAN_BENEFIT, FEE_PLAN_TIP_TXT, CURRENCY_UNIT, MYT_JOIN_CONTRACT_TERMINAL, UNIT} from '../../../../types/string.type';
...
const limitMinMax = (num, min = 0, max = 100): any => {
  if ( num < min ) {
    return min;
  }
  if ( num > max ) {
...
class MyTJoinMyplan extends TwViewController {
  ...
  render(req: Request, res: Response, next: NextFunction, svcInfo: any, allSvc: any, childInfo: any, pageInfo: any) {
    ...
    Observable.combineLatest(
      ...
      try {
        ...
        /*
        let joinProds;
        if (joinProduct && Object.keys(joinProduct).length > 0) {
          joinProds = joinProduct;
          joinProds.isAddProduct = true;
          switch (this._svcType) {
            case 2: // T-PocketFi, T-Login
            case 3: // T-WiBro
              joinProds.addTotCnt = joinProds.addProdCnt;
              break;
            case 4: // 유선
              joinProds.addTotCnt = joinProds.additionCount;
              break;
            default: // 0: 모바일, 1: PPS
              if (joinProds.productCntInfo) {
                joinProds = joinProds.productCntInfo;
              }
              joinProds.addTotCnt = parseInt(joinProds.addProdPayCnt, 10) + parseInt(joinProds.addProdPayFreeCnt, 10);
              // 스마트콜Pick 상품이 있는 경우
              if (additionProds.smartCallPickProds.length > 0) {
                // 가입된 부가상품모두에서 스마트콜pick 옵션상품 제외
                joinProds.addTotCnt = additionProds.additions.length - additionProds.smartCallPickProds.length;
              }
              break;
          }
          // 옵션/할인프로그램 개수
          if (feePlan.dcPrograms) {
            joinProds.addTotCnt = joinProds.addTotCnt + feePlan.dcPrograms.length;
          }
        }
        */
        // 혜택/할인 개수
        // @benefit.my-benefit.controller.ts: count
   ...
   * @param {Array<Object>} dcPrograms - 옵션 및 할인프로그램 목록
   * @return {Array<Object>}
   */
   ...
   */
  private _convertBasDataTxt(basDataGbTxt: any, basDataMbTxt: any): any {
  ...
  // 약정할인/기기상환정보
  private _convertFeeAgreementInfo(priceList: Array<any>, tablet: any /*, wibro: any */) {
  ...
  // 단말기 약정할인 정보
  private _convertTerminalAgreementInfo(tAgree: any, rsvPenTAgree: any, tInstallment: any, sucesAgreeList: Array<any>) {
    ...
    if ( getSize(tInstallment) > 0 ) { // T약정 할부지원
      ...
      this._proDate(tInstallment, tInstallment.tInstallmentOpDt, tInstallmentEndDt);
    ...
    if ( getSize(sucesAgreeList) > 0 ) { // 단말기 승계 정보
      ...
      for ( let i = 0; i < sucesAgreeList.length; i++ ) {
        ...
        * 해당분류에 포함되지않는 경우 | suc_noType
        ...
        this._proDate(sucesAgreeList[i],
  ...
  // 단말기 분할 상환 정보
  private _convertRepaymentAgreementInfo(installmentList: Array<any>) {
  ...
  // // 단말기 구매정보(T렌탈)
  private _convertTRentalAgreementInfo(tRental: any): any {
    let rentalInfo: any = {};
    ...
    if (this._svcType === 0 || this._svcType === 2) {
      return this.apiService.request(API_CMD.BFF_05_0063, {}, null, [], API_VERSION.V2).map((resp) => {
        if (resp.code === API_CODE.CODE_00) {
          ...
          return {
            // 요금 약정 할인
            fees: this._convertFeeAgreementInfo(data.priceList, data.tablet),
            // 단말기(약정할인)
            terminal: this._convertTerminalAgreementInfo(data.tAgree, data.rsvPenTAgree, data.tInstallment,
                data.sucesAgreeList),
            // 단말기 분할상환 정보
            repayment: this._convertRepaymentAgreementInfo(data.installmentList),
            // 단말기 구매정보(T렌탈)
            tRental: this._convertTRentalAgreementInfo(data.tRental)
          };
        }
        // error
        return resp;
  ...
  // 나의 가입 부가 / 결합 상품
  private _getJoinProduct(): Observable<any> {
    let command;
    switch (this._svcType) {
      case 2: // T-PocketFi, T-Login
      case 3: // T-WiBro
        ...
        if (resp.code === API_CODE.CODE_00) {
          ...
          if (data.data && data.data.length) {
            ...
            usagePattern.totalUsage = data.data.reduce((acc, curr) => {
               ...
               */
              // console.log('$$$$$data', curr.invMth, curr.totalUsage);
            ...
            usagePattern.totalUsage = data.voice.reduce((acc, curr) => {
               ...
               * item.unlmtType: 무제한 타입 (1: 데이터, 2: SMS, 3: 데이터 및 SMS)
               */
              // console.log('$$$$$voice', curr.invMth, curr.totalUsage);
              return acc + parseInt(curr.totalUsage, 10) || 0;
            }, 0);
            // 3개월 평균 계산 (x / 3)
            // usagePatterns.aveUsage = Number((usagePatterns.totalUsage / 3).toFixed(2)); // floor
            ...
            usagePattern.totalUsage = data.sms.reduce((acc, curr) => {
               ...
               * item.unlmtType: 무제한 타입 (1: 데이터, 2: SMS, 3: 데이터 및 SMS)
               */
              // console.log('$$$$$sms', curr.invMth, curr.totalUsage);
              return acc + parseInt(curr.totalUsage, 10) || 0;
            }, 0);
            // 3개월 평균 계산 (x / 3)
            // usagePatterns.aveUsage = Number((usagePatterns.totalUsage / 3).toFixed(2)); // floor
