...
import { API_CMD, API_CODE, API_METHOD, API_SERVER, API_VERSION } from '../types_en/api-command.type';
...
import { LINE_NAME, LOGIN_TYPE } from '../types_en/bff.type';
...
class ApiService {
          ...
          if ( !!req.baseUrl && !(req.baseUrl.indexOf('bypass') !== -1 || req.baseUrl.indexOf('native') !== -1 
            ...
            if ( respData.code === API_CODE.BFF_0003 ) {
              ...
              if ( !FormatHelper.isEmpty(loginCookie) && loginCookie === 'Y' ) {
                ...
                res.render('en.error.login-block.html', { target: this.loginService.getPath(req) });
              }
              return;
              ...
              this.printErrorLog('[EN>API RESP] BFF Block', req, command, options, resp);
  ...
  private handleError(observer, command, req, res, options, err) {
    if ( !FormatHelper.isEmpty(err.response) && !FormatHelper.isEmpty(err.response.data) ) {
      ...
      this.printErrorLog('[EN>API ERROR]', req, command, options, error);
          ...
          if ( contentType.includes('json') ) {
            ...
            if ( !!req.baseUrl && !(req.baseUrl.indexOf('bypass') !== -1 
              ...
              if ( !FormatHelper.isEmpty(error.code) && error.code === API_CODE.BFF_0003 ) {
                ...
                this.logger.error(this, '[EN>API RESP] BFF Block', resp.code, resp.msg);
                this.printErrorLog('[EN>API RESP] BFF Block', req, command, options, resp);
                const path = this.loginService.getFullPath(req);
                if ( !(/\/en\/main\/home/.test(path) || /\/en\/main\/store/.test(path) || /\/submain/.test(path)) ) {
                  this.checkServiceBlock(resp.result);
                }
              }
            // client에서 API 직접 호출 시 BFF server session이 변경되었을 경우
      ...
      this.printErrorLog('[EN>API ERROR] Exception', req, command, options, err);
  ...
  private checkServiceBlock(block) {
    const blockUrl = block.fallbackUrl || '/en/common/util/service-block';
  ...
  private redirectInvalidSession(req, res, resp) {
    ...
    res.redirect('/en/common/member/logout/expire?' + params);
  }
