...
import { NODE_ERROR_MSG } from '../types_en/string.type';
...
import { TARGET_PARAM_VALIDATION_CHECK_URL } from '../types_en/url.type';
...
class ErrorService {
  ...
  public render(res: Response, options: ErrorOptions): any {
    return res.render('en.error.server-error.html', {
    ...
    if (req.accepts(['html', 'json']) === 'html' && !FormatHelper.isEmpty(target)) {
      ...
      redisService.getData(REDIS_KEY.URL_META + target.replace(domain, '')).subscribe((resp) => {
        if (resp.code !== API_CODE.REDIS_SUCCESS ) {
          res.render('en.error.server-error.html', {
                svcInfo: null, 
