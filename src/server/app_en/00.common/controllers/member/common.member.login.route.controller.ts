  ...
  render(req: Request, res: Response, next: NextFunction, svcInfo: any, allSvc: any, childInfo: any, pageInfo: any) {
    ...
    if ( !FormatHelper.isEmpty(query.error) ) {
      if ( query.error === '3541' || query.error === '3602' || query.error === '4503' ) {
        ...
        // ID 찾기 클릭
        res.redirect('/en/common/tid/find-id?target=' + params.target);
      } else if ( query.error === '3112' ) {
        // 비밀번호 찾기 클릭
        res.redirect('/en/common/tid/change-pw?target=' + params.target);
      } else if ( query.error === '3113' ) {
        // 회원가입 클릭
        res.redirect('/en/common/tid/signup-foreigner?target=' + params.target);
      } else {
