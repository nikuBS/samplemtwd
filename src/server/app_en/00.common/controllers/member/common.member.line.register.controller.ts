...
import { LINE_NAME, SVC_ATTR_E, SVC_ATTR_NAME } from '../../../../types_en/bff.type';
  ...
  render(req: Request, res: Response, next: NextFunction, svcInfo: any, allSvc: any, childInfo: any, pageInfo: any) {
    ...
    this.getLineInfo().subscribe((resp) => {
      if ( resp.code === API_CODE.CODE_00 ) {
        ...
        const parseData = this.parseLineInfo(resp.result);
        const lineData = {
          data: parseData,
          type: type,
          totalCnt: resp.result.mCnt
        };

        res.render('member/en.common.member.line.register.html', { lineData, svcInfo, pageInfo, landing });
      } else {
